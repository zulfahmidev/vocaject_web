<template>
  <router-link :to="{name: 'progress-project', params: {id: data?.id}}" :class="`block rounded shadow p-3 button-hover ${(isActive) ? 'button-active' : 'bg-white'}`">
...
export default {
  ...
  mounted() {
    ...
    this.isActive = (id == this.data?.id) ? true : false;
  },
}
</script>